...
#-Replace the recruitment value in 2009 with a geometric mean recruitment over the years 1986 to 2007
WoS@stock.n[1,ac(2009)] <- exp(mean(log(rec(WoS[,ac(1989:2007)])),na.rm=T))
...
do.retrospective.plots<- function(stck,idxs,ctrl,n.retro.yrs) {
    ...
    stcks[[9]]@stock.n[1,ac(2009)] <- exp(mean(log(rec(stck[,ac(1989:2007)])),na.rm=T))
...
catch.curves(WoS,1990,2009)
WoS.sr <- ref.pts(WoS,"segreg",100000)
cor.tun(WoS.tun)

#Time series of west
west.ts  <- xyplot(data~year,data=window(WoS@stock.wt,1991,2009),
              groups=age,
...
west.by.cohort      <- as.data.frame(FLCohort(window(WoS@stock.wt,1992,2009)))
