...
t <-
  wt %>% 
  filter(grepl("Total herring Catch \\(NSAS\\)$|Total herring Catch \\(WBSS\\)$", variable)) %>% 
  filter(year >= 1992, year <= 2020) %>% 
  mutate(variable = gsub("Total herring Catch \\(|\\)","", variable)) %>% 
  dplyr::select(-varnr) %>% 
  group_by(year) %>% 
...
plist[[2]] <-
  t %>% 
    ...
    theme(plot.margin=unit(c(0.1,0.1,0.1,0.1), "cm")) +
    geom_bar(aes(fill=variable), stat="identity", position="fill", alpha=1.0) +
    geom_hline(data=tt, aes(yintercept=prop), linetype="dashed", colour="black")+
    geom_text(data=tt, aes(x=2020, y=prop, label=scales::percent(prop, accuracy=1)), colour="black", vjust=0, hjust=1, nudge_y = 0.02)+
    scale_y_continuous(labels=scales::percent) +
    labs(x="",y="proportion") 
...
_Figure xx: Total NSAS and WBSS herring catch. Top: tonnes, Bottom: proportions._
...
t2 %>%  
  ...
  labs(x="catch (tonnes)", y="herring bycatch (tonnes)") +
