...
The Sum of Products (SoP, catch in numbers * weight at age, in '000 tons) of WBSS herring caught in the Transfer area by quarter (columns), age (rows) and year (x-axis) indicates that the majority of the WBSS catches take place in the 2nd and 3rd quarter with ages between 2-7. In recent years, there appears to be an increase in catches of WBSS herring in the transfer area. 
...
**SoP of WBSS herring in transfer area compared to overall catch of all herring in all areas**

The catches of WBSS herring in the transfer area constitute only a small proportion of the overall herring catch in all areas. 
...
_Figure 9:SoP of WBSS herring in the transfer area compared to the overall herring catch in all areas._
...
t2 %>%
  ...
  filter(year >= 2012) %>%
  mutate(propintransfer = sop/value) %>% 

  ggplot(aes(x=year, y=propintransfer, group=1)) +
  theme_publication() +
  geom_bar(stat="identity") +
  scale_y_continuous(labels = scales::percent) +
  expand_limits(y=0)
...
```
_Figure 10:SoP of WBSS herring in the transfer area compared to the total catch of WBSS herring._
...
**Herring catch by rectangle, year and quarter in the North Sea, Eastern Channel, Skagerrak and Western Baltic **
...
_Figure 12b.1: Catch of herring (tonnes) in 2009-2014 by quarter._
...
_Figure 12b.2: Catch of herring (tonnes) in 2015-2020 by quarter._
...
_Figure 12d: Catch of herring (tonnes) in the transfer area (T) compared to the rest of the eastern North Sea (NT) by year and quarter._
...
t <-
  ...
  filter(species=="HER") %>% 
  filter(year %in% myyears) %>% 
  group_by(year, quarter, rect, lat, lon) %>% 
  summarise(catch = sum(catch, na.rm=TRUE)) %>% 
  ungroup() %>% 
  ...
  filter(!is.na(catch_interval)) %>% 
  filter(grepl("\\,", catch_interval)) %>%   
  group_by(year, quarter, rect, lat, lon, catch_interval) %>% 
  summarize(catch = sum(catch, na.rm=TRUE))
...
tt <-
  ...
  group_by(year, quarter) %>% 
  summarise(catch = sum(catch, na.rm=TRUE)) %>% 
  mutate(str = catch/1000) %>% 
  mutate(str = paste(as.integer(str), "kt", sep=" "))
...
_Figure 2e: Catch of herring (tonnes) in 3a-c by year and quarter._
