...
NSH.tun   <- readFLIndices(file.path(data.source,"fleetLAI.txt"))
NSH.tun   <- lapply(NSH.tun,function(x) {x@type <- "number"; return(x)})
NSH.tun[["IBTS0"]]@range["plusgroup"] <- NA
NSH.tun[["LAI"]]@range["startf"] <- NA
NSH.tun[["LAI"]]@range["endf"] <- NA
...
#NSH@catch.n[1,ac(1978:1979)]          <- c(347500.7,783993.1)
