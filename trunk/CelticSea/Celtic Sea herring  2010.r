...
retro.years         <-  c(2003,2005:2009)  #Specify specific years to do the retrospective over
...
################################################################################
## Change Recruitment to mean value 1995-2007
...
# puts the geomean value into the pop numbers at age 1 in 2009

cs.herring.ica@stock.n['1',(as.character(cs.herring@range['maxyear'])),,,,]=Rec

##Need to adjust the survivors also
gm.recs <- exp(mean(log(rec(trim(cs.herring,year=1995:2007)))))
...
cs.herring.ica@survivors['2',ac(2010)]=  cs.herring.stf@stock.n['2',ac(2010)]
...
## TAC years specified
## Catch all years
TACs    <- data.frame(year=1974:2010,TAC=1000*c(32,25,10.8,0,0,6,6,6,8,8,13,13,17,18,18,20,17.5,21,21,21,21,21,21,22,22,21,21,20,11,13,13,13,11,9.4,7.9,5.9,10.15))
...
#Time series of west
west.ts  <- xyplot(data~year,data=window(cs.herring@stock.wt,1991,2009),
              groups=age,
...
catch.curves(cs.herring,1990,2009)
