...
cat("GENERATING SUMMARY PLOTS ...\n");flush.console()
  ...
  summary.data <- as.data.frame(FLQuants(SSB=ssb(stck)[,9:38],"Fbar4-8"=fbar(stck)[,6:38],Recruits=1000*rec(stck),Catch=catch(stck)))
  ...
  panel=function(...) {
  panel.grid(h=-1,v=-1)
    if(panel.number()==1)   panel.xyplot(...,col="black",main="Catch (t)")
  if(panel.number()==4) {  panel.xyplot(...,col="black",main="SSB (t)")
                           panel.abline(h= 2.3,lty="dotted")
                           panel.abline(h= 1.7,lty="dashed")
                           panel.text(1972,1.7+0.25," Blim",cex=0.8)
                           panel.text(1972,2.3+0.25,"Bpa",cex=0.8)
   }
  if(panel.number()==2) {  panel.xyplot(...,col="black")  
...
retro.dat$data[retro.dat$value=="Recruits"]=1000*retro.dat$data[retro.dat$value=="Recruits"]
