...
PARAMETER_SECTION
  ...
  init_number logScaleSSB(-1);                                                  //Change for Irish Sea herring
  init_number logPowSSB(ssbPowPhase);
  //init_number logSdSSB(ssbPhase);  
  init_number logSdSSB(-1);                                                     //Change for Irish Sea herring
      ...
      if(ft==1){// comm fleet
        ...
        if(ft==2){// survey
          ...
          if(ft==3){// SSB survey
            ...
            pred(i)=log(ssb)+log(1.2);//+logScaleSSB;                                    //Change for Irish Sea herring
          }else{
            if(ft==4){
              dvariable ssb=0.0; 
              for(int a=minAge; a<=maxAge; ++a){
              ...
              pred(i)=exp(logPowSSB)*log(ssb);//+logScaleSSB;                   //Change for Irish Sea herring
            }
          }
        } 
      }
  ...
  for(int i=n1; i<=n2; ++i){
    ...
    if((retro(f)==-1)||((int)data(i,1)>lastYearData(f))){
      ...
      if((fleetTypes(f)==3)||(fleetTypes(f)==4)){
        //var=exp(2.0*logSdSSB);                                                //Change for Irish Sea herring                   
        var=0.05;
      }else{ 
