...
#- Set starting values (for fixed parameters)
startFpar     <- rep(-5,length(unique(ISH.ctrl@catchabilities[ISH.ctrl@catchabilities>=0])))
startFpar[ISH.ctrl@catchabilities["VIIaNSpawn",1]+1] <- log(1)
mapFpar       <- startFpar; mapFpar[ISH.ctrl@catchabilities["VIIaNSpawn",1]+1] <- NA
startObsVar   <- rep(-0.35,length(unique(ISH.ctrl@obs.vars[ISH.ctrl@obs.vars>=0])))
startObsVar[ISH.ctrl@obs.vars["VIIaNSpawn",1]+1] <- log(0.4)
mapObsVar     <- startObsVar; mapObsVar[ISH.ctrl@obs.vars["VIIaNSpawn",1]+1] <- NA

#- Run final model
ISH.sam       <- FLSAM(ISH,ISH.tun,ISH.ctrl,
                       starting.values=list(logFpar=startFpar,logSdLogObs=startObsVar),
                       map=list(logFpar=as.factor(mapFpar),logSdLogObs=as.factor(mapObsVar)))
                       
#- Run a retro
ISH.retro     <- new("FLSAMs")
for(iYr in 2017:2012){
  rISH      <- window(ISH,end=iYr)
  rISH.tun  <- window(ISH.tun,end=iYr)
  source(file.path(my.path,"setupControl.r"))
  rISH.ctrl <- ISH.ctrl
  rISH.ctrl@range["maxyear"] <- iYr
  rISH.ctrl@residuals <- F
  ISH.retro[[ac(iYr)]] <- FLSAM(rISH,rISH.tun,rISH.ctrl,starting.values=list(logFpar=startFpar,logSdLogObs=startObsVar),
                       map=list(logFpar=as.factor(mapFpar),logSdLogObs=as.factor(mapObsVar)))
}
...
# calc mohns-rho
mean(mohns.rho(ISH.retro,ref.year=2017,span=5,type="fbar")[1:5,1])
mean(mohns.rho(ISH.retro,ref.year=2017,span=5,type="ssb")[1:5,1])
mean(mohns.rho(ISH.retro,ref.year=2017,span=5,type="rec")[1:5,1])
plot(ISH.retro)
