...
# name of the run
# exactly as specified in the run folder

run <- '2017 ASSESSMENT FINAL'

# first age 
firstage <- 1


#dir1<-("L:/PERSONAL/Afra/ASAP/input files for exploratory runs Nov 2016/dummy data/")
#dir2<-("L:/PERSONAL/Afra/ASAP/input files for exploratory runs Nov 2016/dummy data/more plots")
...
for(i in 1:n){
  j <- which(REP==paste(' Index number',i))
  ind0 <- read.table(REPfile,header=F,skip=j,nrows=118)[,-c(1,4)]
...
res1<-subset(res1,year %in% seq(2002,2016))
...
#res1b<-subset(res1, year==2002:2016)
res1b<-subset(res1,year %in% seq(2002,2016))
