...
# 12/03/2019 Improved readability of the code (MP)
#-------------------------------------------------------------------------------
...
# rename objects (to make them consistent)
NSH            <- NSH
NSH.ctrl       <- NSH.ctrl
NSH.sam        <- NSH.sam
NSH.tun        <- NSH.tun
...
NSHmf          <- NSHs3
NSHmf.ctrl     <- NSH3.ctrl
NSHmf.sam      <- NSH3f.sam

rm(NSHs3, NSH3.ctrl, NSH3f.sam)

#-------------------------------------------------------------------------------
# functions for scenarios
#-------------------------------------------------------------------------------

stf.table.option <- function(option, stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty){
  
  stf.table.[option,"Fbar 2-6 A",] <- 
    iterQuantile(quantMeans(stf.@harvest[f26,FcY,"A"]))
  
  stf.table.[option,grep("Fbar 0-1 ",dimnames(stf.table.)$values),]  <- 
    aperm(iterQuantile(quantMeans(stf.@harvest[f01,FcY,c("B","C","D")])),c(2:6,1))
  
  stf.table.[option,"Fbar 2-6",] <- 
    iterQuantile(quantMeans(unitSums(stf.@harvest[f26,FcY,])))
  
  stf.table.[option,"Fbar 0-1",] <- 
    iterQuantile(quantMeans(unitSums(stf.@harvest[f01,FcY,])))
  
  stf.table.[option,grep("Catch ",dimnames(stf.table.)$values),] <- 
    aperm(iterQuantile(harvestCatch(stf.,FcY)),c(2:6,1))
  
  stf.table.[option,grep("SSB",dimnames(stf.table.)$values)[1],] <- 
  ...
  stf.table.[option,grep("SSB",dimnames(stf.table.)$values)[2],] <- 
    iterQuantile(ssb.CtY)
...
stf.calculate.catch <- function(dms.=dms, stf.=stf, RECS.=RECS, y=CtY){
  ...
  # ssb = N * w * mat * exp(-F*fprop - M*mprop); assume same harvest as in FcY
  ssb.CtY                                                     <- 
    quantSums(stf.@stock.n[,y,1] * stf.@stock.wt[,y,1]*stf.@mat[,y,1]*
              exp(-unitSums(stf.@harvest[,FcY])*stf.@harvest.spwn[,y,1]-stf.@m[,y,1]*stf.@m.spwn[,y,1])) 
  
  return(list(stf., ssb.CtY))
...
# Update NSH object with stock numbers and fishing mortality (from single fleet run)
...
# TAC information for 2019
# TACNSA      <- 385008  # taken from EU Council regulation 2019/124; HER/4AB. + HER/4CXB7D
# TACNSB      <- 13190   # taken from TAC regulation document HER/2A47DX
# TAC3aC      <- 29326   # HER/03A. Split
# TAC3aD      <- 6659    # HER/03A-BC
...
TACS[,,"C"] <- c(TAC3aC*(1-Ctransfer)*Csplit,
                 5557/(1-Csplit), #TAC3aC*(1-Ctransfer)*Csplit,   # Q? Where is the 5557 coming from?
                 5557/(1-Csplit))
TACS[,,"D"] <- c(TAC3aD*Dsplit*Duptake,
                 332/(1-Dsplit),#TAC3aD*Dsplit*Duptake,           # Q? Where is the 332 coming from?
...
# Recruitments; in ImY taken from assessment output
# For forecast and continuation year the weighted geometric mean of the 10 years before
RECS        <- 
  FLQuants("ImY" =FLQuant(subset(rec(NSH.sam),year==ImY)$value,
                          dimnames=list(age=dimnames(NSH)$age[1],
                                        year=ImY,
                                        unit="unique",
                                        season="all",
                                        area="unique",
                                        iter=1:dims(stk)$iter)),
...
for(i in 1:length(dms$unit)){
  ...
  if(i == 4) {
    stf@harvest[,FuY,4]     <- NSHmf.sam@harvest[,ImY,,,2]
    stf@catch.wt[,FuY,4]    <- NSHmf[["residual"]]@catch.wt[,DtY,,,2]
    stf@landings.wt[,FuY,4] <- NSHmf[["residual"]]@catch.wt[,DtY,,,2]
  }
  
  # Fill other fleets with their respective fishing mortalities and weights
  if(i != 4) {
    stf@harvest[,FuY,i]     <- NSHmf.sam@harvest[,ImY,,,i]
    stf@catch.wt[,FuY,i]    <- NSHmf[["residual"]]@catch.wt[,DtY,,,i]
    stf@landings.wt[,FuY,i] <- NSHmf[["residual"]]@catch.wt[,DtY,,,i]
  }
}
...
# Get stock numbers from SAM output; the same for each fleet
for(i in dms$unit)
...
# Calculate fishing mortality by fleet in the intermediate year
# Function 'fleet.harvest' is in 'stfFunctions.r'. 
stf@harvest[,ImY]         <- fleet.harvest(stk=stf,iYr=ImY,TACS=TACS[,ImY])

# For each fleet, calculate catch.n, catch and landings.n, landings
for(i in dms$unit){
...
# Calculate Fbar 2-6 for fleet A in ImY
stf.table[1,"Fbar 2-6 A",]    <- 
  iterQuantile(quantMeans(stf@harvest[f26,ImY,"A"]))

# Calculate Fbar 0-1 for fleets B, C and D in ImY
stf.table[1,grep("Fbar 0-1 ",dimnames(stf.table)$values),]  <- 
  aperm(iterQuantile(quantMeans(stf@harvest[f01,ImY,c("B","C","D")])),c(2:6,1))

# Calculate overall Fbar 2-6 for ImY
stf.table[1,"Fbar 2-6",]  <- 
  iterQuantile(quantMeans(unitSums(stf@harvest[f26,ImY,])))

# Calculate overall Fbar 0-1 for ImY
stf.table[1,"Fbar 0-1",]  <- 
  iterQuantile(quantMeans(unitSums(stf@harvest[f01,ImY,])))

# Calculate Catch in ImY
stf.table[1,grep("Catch ",dimnames(stf.table)$values),] <- 
  aperm(iterQuantile(harvestCatch(stf,ImY)),c(2:6,1))

# Calcualte SSB in ImY
stf.table[1,grep("SSB",dimnames(stf.table)$values)[1],] <- 
  iterQuantile(quantSums(stf@stock.n[,ImY,1] * stf@stock.wt[,ImY,1] *
                         exp(-unitSums(stf@harvest[,ImY])*
                                       stf@harvest.spwn[,ImY,1]-stf@m[,ImY,1]*
                                       stf@m.spwn[,ImY,1]) * stf@mat[,ImY,1]))

# Calculate Fsq in ImY at 2-6
...
# Calculate survivors using harvest in ImY
for(i in dms$unit) stf@stock.n[2:(dims(stf)$age-1),FcY,i]   <- 
  (stf@stock.n[,ImY,1]*exp(-unitSums(stf@harvest[,ImY])-stf@m[,ImY,1]))[ac(range(stf)["min"]:(range(stf)["max"]-2)),]

# Calculate plusgroup
for(i in dms$unit) stf@stock.n[dims(stf)$age,FcY,i]         <- 
  apply((stf@stock.n[,ImY,1]*exp(-unitSums(stf@harvest[,ImY])-stf@m[,ImY,1]))[ac((range(stf)["max"]-1):range(stf)["max"]),],2:6,sum,na.rm=T)
...
if("fmsyAR" %in% stf.options){
  ...
  #reset harvest in FcY for all fleets
  ...
  for(iTer in 1:dims(stf)$iter)      #stk.=stk,rec.=rec,f.=fmsy,f26.=f26,f01.=f01,TACS.=TACS
    res[,iTer]                  <- 
       nls.lm(par=rep(1,3),
              ...
              refs.=referencePoints,
              jac=NULL,
              nls.lm.control(ftol = (.Machine$double.eps),maxiter = 1000))$par[c(1,1,2,3)]
  
  stf@harvest[,FcY]             <- sweep(stf@harvest[,FcY],c(3,6),res,"*")
  #-end 2018------------

  # Calculate catch and landings
  stf <- stf.calculate.catch (dms.=dms, stf.=stf, y=FcY)
  ...
  # Estimated B-fleet catch for FcY & CtY from F MSY Advice Rule option added afterwards
  # previously this was taken from the MP option, but there is currently no MP in place
  BcatchMP    <- stf.table["fmsyAR","Catch B","50%"]

  # Update the TACs for the B fleet for subsequent analysis 
  TACS[,,"B"] <- c(TACNSB * Buptake,
                   BcatchMP ,
                   BcatchMP); 
  
  # temporary store the TACS object
  TACSlst     <- TACS
  
}

### (Old Management Plan) using find.FABCD function ### ------------------------------------------------------
...
if("mp" %in% stf.options){
  ...
  # Use find.FABCD to find fleet options according to (old) management plan
  for(iTer in 1:dims(stf)$iter)      #stk.=stk,rec.=rec,f.=fmsy,f26.=f26,f01.=f01,TACS.=TACS
    res[,iTer]                  <- optim(
                                       par       = rep(1,4),
                                       find.FABCD,
...
#  #- Calculate catches based on new harvest pattern
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="mp", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("nf" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="nf", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("+15%" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="+15%", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
### TACRO TAC Roll Over. Same TAC for A-fleet as last year ### ----------------------------------------------------------
...
if("tacro" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="tacro", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("fmsy" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="fmsy", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("fpa" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="fpa", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("flim" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="flim", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("fsq" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="fsq", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("bpa" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="bpa", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("blim" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="blim", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
...
if("MSYBtrigger" %in% stf.options){
  ...
  # Calculate the values to be included in the stf.table
  stf.table <- stf.table.option (option="MSYBtrigger", stf.table.=stf.table, stf.=stf, ssb. = ssb.Cty)
