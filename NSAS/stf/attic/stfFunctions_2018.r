...
find.FAB      <- function(mult,stk.=stk,f01.=f01,f26.=f26,mp.options.=mp.options,TACS.=TACS){
                    ...
                    if(mp.options. == "i"){
                      ...
                      fbarB <- mean(bigF[f01.,])
                      fbarA <- mean(bigF[f26.,])
                      #ret <- -1*c(dnorm(log(fbarA),log(resA),log=T),dnorm(log(fbarB),log(resB),log=T),dnorm(log(catchC),log(c(TACS.)[3]),log=T),dnorm(log(catchD),log(c(TACS.)[4]),log=T))
                      ret   <- c(sqrt(c((fbarA-resA)^2,(fbarB-resB)^2,(catchC - c(TACS.)[3])^2,(catchD - c(TACS.)[4])^2)))
                        ...
                        catchC    <- sum(stk.@stock.n[,,"C"]*(1-exp(-unitSums(stk.@harvest)-stk.@m[,,"C"]))*(stk.@harvest[,,"C"]/(unitSums(stk.@harvest)+stk.@m[,,"C"]))*stk.@catch.wt[,,"C"])
                        catchD    <- sum(stk.@stock.n[,,"D"]*(1-exp(-unitSums(stk.@harvest)-stk.@m[,,"D"]))*(stk.@harvest[,,"D"]/(unitSums(stk.@harvest)+stk.@m[,,"D"]))*stk.@catch.wt[,,"D"])
                        ret <- -1*c(dnorm(log(fbarA),log(resA),log=T),0,dnorm(log(catchC),log(c(TACS.)[3]),log=T),dnorm(log(catchD),log(c(TACS.)[4]),log=T))
...
find.F <- function(mult,stk.=stk,f.=fmsy,f26.=f26,f01.=f01,TACS.=TACS){
                    stk.@harvest        <- sweep(stk.@harvest,3,c(rep(mult[1],2),mult[2],mult[3]),"*")
                    stkZ                <- unitSums(stk.@harvest) + stk.@m[,,1]
                    resA                <- sqrt(c(f. - quantMeans(unitSums(stk.@harvest[f26.,])))^2)*100
                    resCD               <- sqrt(c((TACS. - c(apply(sweep(stk.@stock.n * stk.@catch.wt * sweep(stk.@harvest,c(1:2,4:6),stkZ,"/"),c(1:2,4:6),(1-exp(-stkZ)),"*"),3:6,sum,na.rm=T)))^2))[3:4]
                    res                 <- c(resA,resCD)
                 return(res)}
...
find.FABC <- function(mult,Fs,Ns,Wts,CWts,fspwns,mspwns,mats,Ms,f01,f26,Tcs,mixprop,WBSScatch){
  ...
  CtargetC          <- ifelse(WBSScatch<=1,c(Tcs[,2,3]),(0.057*sum(catchABCD[1]) + 0.41*WBSScatch) * mixprop)
  bigF              <- rowSums(Fs)
  ...
  ret <- -1*c(dnorm(log(fbarA),log(resA),log=T),dnorm(log(fbarB),log(resB),log=T),dnorm(log(catchABCD[3]),log(CtargetC),log=T),dnorm(log(catchABCD[4]),log(Tcs[,2,4]),log=T))
...
find.FABC_F <- function(mult,Fs,Ns,Wts,CWts,fspwns,mspwns,mats,Ms,f01,f26,Tcs,mixprop,FA,FB){
  ...
  for(i in 1:length(mult))
    catchABCD[i]    <- sum(Ns*(1-exp(-rowSums(Fs)-Ms))*(Fs[,i]/(rowSums(Fs)+Ms))*CWts[,i])
  CtargetC          <- ifelse(c(Tcs[,2,3])<=1,c(Tcs[,2,3]),(0.057*sum(catchABCD[1]) + 0.41*WBSScatch) * mixprop)
  bigF              <- rowSums(Fs)
  fbarA             <- mean(bigF[f26])
  fbarB             <- mean(bigF[f01])
  ...
  meanCC            <- mean(catchABCD[3],CtargetC)
  meanCD            <- mean(catchABCD[4],c(Tcs[,2,4]))
  ret <- -1*c(dnorm(log(fbarA),log(FA),log=T),dnorm(log(fbarB),log(FB),log=T),dnorm(log(catchABCD[3]),log(CtargetC),log=T),dnorm(log(catchABCD[4]),log(Tcs[,2,4]),log=T))
...
find.FABCD <- function(mult,Fs,Ns,Wts,CWts,fspwns,mspwns,mats,Ms,f01,f26,Tcs,Tcsorig,mixprop,WBSScatch){
  ...
  catchABCD         <- numeric(length(mult))#; catchABCD[] <- NA
  for(i in 1:length(mult))
    catchABCD[i]    <- sum(Ns*(1-exp(-rowSums(Fsm)-Ms))*(Fsm[,i]/(rowSums(Fsm)+Ms))*CWts[,i])
  ...
  if(invokeCatchIAV){
    cat(c(targetA,catchABCD[2:4]))
    cat("\n")
    #res       <- optim(par=mult,fn=rescaleFF,Fs=Fsm,Ns=Ns,Wts=Wts,CWts=CWts,Ms=Ms,catches=c(targetA,catchABCD[2:4]),lower=rep(1e-4,length(mult)),method="L-BFGS-B")
    res       <- nls.lm(par=mult,lower=rep(1e-08,4), upper=NULL,rescaleFF,Fs=Fsm,Ns=Ns,Wts=Wts,CWts=CWts,Ms=Ms,catches=c(targetA,catchABCD[2:4]),jac=NULL,nls.lm.control(ftol = (.Machine$double.eps),maxiter = 1000))
    cat(res$par)
    cat("\n")

    Fsnew     <- sweep(Fsm,2,res$par,"*")
  ...
  if(invokeFIAV == FALSE & invokeCatchIAV == FALSE)
    ret <- -1*c(dnorm(log(fbarA),log(resA),log=T),dnorm(log(fbarB),log(resB),log=T),dnorm(log(catchABCD[3]),log(CtargetC),log=T),dnorm(log(catchABCD[4]),log(c(Tcs[,2,4])),log=T))
    #ret               <- sqrt(c((fbarA - resA)/resA,(fbarB-resB)/resB,(catchABCD[3] - CtargetC)/(CtargetC+1e-4),(catchABCD[4] - c(Tcs[,2,4]))/c((Tcs[,2,4]+1e-4)))^2)
  if(invokeFIAV == FALSE & invokeCatchIAV == TRUE)
    ret <- -1*c(dnorm(log(catchABCD[1] ),log(targetA),log=T),dnorm(log(fbarB),log(resB),log=T),dnorm(log(catchABCD[3]),log(CtargetC),log=T),dnorm(log(catchABCD[4]),log(c(Tcs[,2,4])),log=T))
    #ret               <- sqrt(c((catchABCD[1] - targetA)/targetA,(fbarB-resB)/resB,(catchABCD[3] - CtargetC)/(CtargetC+1e-4),(catchABCD[4] - c(Tcs[,2,4]))/(Tcs[,2,4]+1e-4))^2)
  if(invokeFIAV == TRUE)
    ret <- -1*c(dnorm(log(fbarA),log(targetA),log=T),dnorm(log(fbarB),log(resB),log=T),dnorm(log(catchABCD[3]),log(CtargetC),log=T),dnorm(log(catchABCD[4]),log(c(Tcs[,2,4])),log=T))
    #ret               <- sqrt(c((fbarA - targetA)/targetA,(fbarB-resB)/resB,(catchABCD[3] - CtargetC)/(CtargetC+1e-4),(catchABCD[4] - c(Tcs[,2,4]))/(Tcs[,2,4]+1e-4))^2)
...
#-------------------------------------------------------------------------------
find.FCAR <- function(mult,stk.=stk,f26.=f26,f01.=f01,TACS.=TACS,WBSScatch=WBSScatch,Csplit=Csplit,refs.){
