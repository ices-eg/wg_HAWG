  ...
  TACS.orig[,,"C"] <- c(TAC3aC,
                        5557/(1-Csplit)/Csplit/(1-Ctransfer), # effective TAC3aC assuming same Ctransfer and Csplit for FcY
                        5557/(1-Csplit)/Csplit/(1-Ctransfer)) # effective TAC3aC assuming same Ctransfer and Csplit for CtY
  ...
  if("fmsyAR" %in% stf.options){
    ...
    # run script
    source(file.path(scriptPath,"fmsyAR.r"))
    
    # update stf table
    stf.table[caseName,"Fbar 2-6 A",]                                  <- quantMeans(stf@harvest[f26,FcY,"A"])
    stf.table[caseName,grep("Fbar 0-1 ",dimnames(stf.table)$values),]  <- aperm(quantMeans(stf@harvest[f01,FcY,c("B","C","D")]),c(2:6,1))
    stf.table[caseName,"Fbar 2-6",]                                    <- quantMeans(unitSums(stf@harvest[f26,FcY,]))
    stf.table[caseName,"Fbar 0-1",]                                    <- quantMeans(unitSums(stf@harvest[f01,FcY,]))
    stf.table[caseName,grep("Catch ",dimnames(stf.table)$values),]     <- aperm(harvestCatch(stf,FcY),c(2:6,1))
    stf.table[caseName,grep("SSB",dimnames(stf.table)$values)[1],]     <- ssb.FcY
    stf.table[caseName,grep("SSB",dimnames(stf.table)$values)[2],]     <- ssb.CtY
  }
  ...
  if("mpA" %in% stf.options){
    ...
    # run script
    source(file.path(scriptPath,"mp.r"))
...
if("+15%" %in% stf.options){
  ...
  TACS[,FcY,"A"]            <- TACS.orig[,ImY,"A"]*1.15 + Ctransfer*TAC3aC
  TACS[,FcY,"B"]            <- TACS.orig[,ImY,"B"]*Buptake
  
  # not sure why the 15% increase applies to the TAC C fleet
  ...
  TACS[,CtY,"A"]            <- c(TACS.orig[,ImY,"A"]*1.15*1.15) + Ctransfer*TAC3aC
  TACS[,CtY,"B"]            <- TACS.orig[,FcY,"B"]*Buptake
  # why do we increase TAC by 15% for the C fleet?
...
if("-15%" %in% stf.options){
  ...
  # 15% decrease only for the A fleet???
  TACS[,FcY,"A"]            <- TACS.orig[,ImY,"A"]*0.85 + Ctransfer*TAC3aC
  TACS[,FcY,"B"]            <- TACS.orig[,ImY,"B"]*Buptake
  
  # same as +15% increase. Why do we apply it to the C fleet
  #TACS                      <- TACSlst #reset catch forecast
  ...
  TACS[,CtY,"A"]            <- c(TACS.orig[,ImY,"A"]*0.85*0.85) + Ctransfer*TAC3aC
  TACS[,FcY,"B"]            <- TACS.orig[,ImY,"B"]*Buptake
  # why do we increase TAC by 15% for the C fleet?
  ...
  # fill in table
  stf.table["-15%","Fbar 2-6 A",]                                  <- quantMeans(stf@harvest[f26,FcY,"A"])
  stf.table["-15%",grep("Fbar 0-1 ",dimnames(stf.table)$values),]  <- aperm(quantMeans(stf@harvest[f01,FcY,c("B","C","D")]),c(2:6,1))
  stf.table["-15%","Fbar 2-6",]                                    <- quantMeans(unitSums(stf@harvest[f26,FcY,]))
  stf.table["-15%","Fbar 0-1",]                                    <- quantMeans(unitSums(stf@harvest[f01,FcY,]))
  stf.table["-15%",grep("Catch ",dimnames(stf.table)$values),]     <- aperm(harvestCatch(stf,FcY),c(2:6,1))
  stf.table["-15%",grep("SSB",dimnames(stf.table)$values)[1],]     <- ssb.FcY
  stf.table["-15%",grep("SSB",dimnames(stf.table)$values)[2],]     <- ssb.CtY
}
...
if("tacro" %in% stf.options){
  ...
  TACS[,FcY,"A"]            <- TACS.orig[,ImY,"A"] + Ctransfer*TAC3aC
  TACS[,FcY,"B"]            <- TACS.orig[,ImY,"B"]*Buptake
  
  # why changing TAC C fleet?
  #if(WBSScatch<=1){
  #  TACS[,FcY,"C"]          <- dummy
  ...
  TACS[,CtY,"A"]            <- c(TACS.orig[,ImY,"A"]) + Ctransfer*TAC3aC
  TACS[,CtY,"B"]            <- TACS.orig[,ImY,"B"]*Buptake
  ...
  stf.table["tacro","Fbar 2-6 A",]                                  <- quantMeans(stf@harvest[f26,FcY,"A"])
  stf.table["tacro",grep("Fbar 0-1 ",dimnames(stf.table)$values),]  <- aperm(quantMeans(stf@harvest[f01,FcY,c("B","C","D")]),c(2:6,1))
  stf.table["tacro","Fbar 2-6",]                                    <- quantMeans(unitSums(stf@harvest[f26,FcY,]))
  stf.table["tacro","Fbar 0-1",]                                    <- quantMeans(unitSums(stf@harvest[f01,FcY,]))
  stf.table["tacro",grep("Catch ",dimnames(stf.table)$values),]     <- aperm(harvestCatch(stf,FcY),c(2:6,1))
  stf.table["tacro",grep("SSB",dimnames(stf.table)$values)[1],]     <- ssb.FcY
  stf.table["tacro",grep("SSB",dimnames(stf.table)$values)[2],]     <- ssb.CtY
}
...
if("fmsy" %in% stf.options){
  ...
  res <- matrix(NA,
                nrow=dims(stf)$unit,ncol=dims(stf)$iter,dimnames=list(dimnames(stf@stock.n)$unit,dimnames(stf@stock.n)$iter))
