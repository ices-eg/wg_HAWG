...
### ======================================================================================================
output.dir          <-  file.path(".","results/4a_LAI/")        # figures directory
...
source(file.path("R/4a_LAI/setupAssessmentObjects_LAI.r"))
source(file.path("R/4a_LAI/setupControlObject_LAI.r"))
...
#SCAI model first
NSHscai.ctrl <- drop.from.control(NSH.ctrl,fleets="LAI-BUN")
NSHscai.tun <- NSH.tun[1:5]
NSHscai.ctrl@obs.vars[3,2:7] <- 102:107
NSHscai.ctrl@obs.vars[5,1:7] <- 200:206
NSHscai.ctrl <- update(NSHscai.ctrl)
NSHscai.sam <- FLSAM(NSH,NSHscai.tun,NSHscai.ctrl)
name(NSHscai.sam) <- "SCAI"
NSHscai.ctrl@residuals <- F
NSHscai.retro <- retro(NSH,NSHscai.tun,NSHscai.ctrl,7)

#LAI model second
NSHlai.ctrl <- drop.from.control(NSH.ctrl,fleets="SCAI")
NSHlai.tun <- NSH.tun[c(1:4,6:9)]
NSHlai.ctrl@obs.vars[3,2:7] <- 102:107
NSHlai.ctrl@obs.vars[5,1:7] <- 200:206
NSHlai.ctrl <- update(NSHlai.ctrl)
NSHlai.sam <- FLSAM(NSH,NSHlai.tun,NSHlai.ctrl)
name(NSHlai.sam) <- "LAI"
NSHlai.ctrl@residuals <- F
NSHlai.retro <- retro(NSH,NSHlai.tun,NSHlai.ctrl,7)
