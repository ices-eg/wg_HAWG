...
### ======================================================================================================
output.dir          <-  file.path(".","results/4b_LAIparameters/")        # figures directory
...
source(file.path("R/4b_LAIparameters/setupAssessmentObjects_LAI.r"))
source(file.path("R/4b_LAIparameters/setupControlObject_LAI.r"))
NSHlai.ctrl <- drop.from.control(NSH.ctrl,fleets="SCAI")
NSHlai.tun <- NSH.tun[c(1:4,6:9)]
...
#base LAI model second
NSHlai.ctrl <- drop.from.control(NSH.ctrl,fleets="SCAI")
NSHlai.tun <- NSH.tun[c(1:4,6:9)]
NSHlai.ctrl@residuals <- F
NSHlai.sam <- FLSAM(NSH,NSHlai.tun,NSHlai.ctrl)
name(NSHlai.sam) <- "LAI"

#Fix logP to 1 parameter
NSHlai.ctrl@logP.vars[] <- 0
NSHlailogp.sam <- FLSAM(NSH,NSHlai.tun,NSHlai.ctrl)
name(NSHlailogp.sam) <- "LAI-logP"

#Fix SD parameters to 1
source(file.path("R/4b_LAIparameters/setupAssessmentObjects_LAI.r"))
source(file.path("R/4b_LAIparameters/setupControlObject_LAI.r"))
NSHlai.ctrl <- drop.from.control(NSH.ctrl,fleets="SCAI")
NSHlai.tun <- NSH.tun[c(1:4,6:9)]
...
NSHlai.ctrl@obs.vars[grep("LAI",rownames(NSHlai.ctrl@obs.vars)),1] <- 101
NSHlai.ctrl <- update(NSHlai.ctrl)
NSHlaiobsv.sam <- FLSAM(NSH,NSHlai.tun,NSHlai.ctrl)
name(NSHlaiobsv.sam) <- "LAI-obsv"
...
NSHlai.ctrl@power.law.exps[grep("LAI",rownames(NSHlai.ctrl@power.law.exps)),1] <- -1
NSHlai.ctrl <- update(NSHlai.ctrl)
NSHlaipl.sam <- FLSAM(NSH,NSHlai.tun,NSHlai.ctrl)
name(NSHlaipl.sam) <- "LAI-nopowerlaw"

sams <- FLSAMs(NSHlai.sam,NSHlailogp.sam,NSHlaiobsv.sam,NSHlaipl.sam)
AIC(sams)
