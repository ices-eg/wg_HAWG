...
# modified at HAWG March 2017 by susan Lusseau

#To do:
# Develop more plots 
#   - total catches versus TAC each year
#
#
####################################################################################################
...
# generating a sam.pin from a working assessmnet
#sampin <- FLSAM(MSH,MSH.tun,MSH.ctrl)
#save(sampin,file="./saved_pin/sampin.Rdata") #To be used in another assessmnet context

# OR run assessmnet using the sampin object
MSH.sam   <-  FLSAM(MSH,MSH.tun,MSH.ctrl,pin.sam = sampin)
...
save(MSH,MSH.sam,MSH.tun,MSH.ctrl,file=file.path(output.dir,"2017_VIaHerring.Rdata"))
...
pdf(paste(output.base,"2017_MSH.pdf",sep=""))
...
plot(MSH) #without the uncertainties
...
plot.otolith(MSH.sam,n=50000, year=2016)
...
save(MSH,MSH.retro,file=file.path(output.dir,"2017_VIaHerring_retro.Rdata"))
