...
if(PDF) pdf(file.path(output.dir,paste(run_name,"_",assessment_name_singlefleet,"_diagnostics_%02d.pdf",sep="")))
if(PNG) png(file.path(output.dir,paste(run_name,"_",assessment_name_singlefleet,"_diagnostics_%02d.png",sep="")),units = "px", height=800,width=672, bg = "white")
residual.diagnostics(MSH.sam)

# figure - assessment result, spawning stock biomass, fishing mortality, recruitment
plot(MSH.sam,futureYrs=F)
...
print(xyplot(value+ubnd+lbnd ~ age | fleet,catch,
       ...
       subset=fleet %in% c("MS HERAS","WoS HERAS","IBTS_Q1","IBTS_Q4"),
...
print(cor.plot(MSH.sam))

# figure - catch residuals per year per age
dat <- subset(residuals(MSH.sam),fleet=="catch unique")
...
# figure - acosutic index residuals per year per age
dat <- subset(residuals(MSH.sam),fleet=="MS HERAS")
...
# figure - acosutic index residuals per year per age
dat <- subset(residuals(MSH.sam),fleet=="WoS HERAS")
print(xyplot(age ~ year,data=dat,cex=dat$std.res,col="black",main="Residuals by year WoS HERAS",
       panel=function(...){
         lst <- list(...)
         panel.xyplot(lst$x,lst$y,pch=ifelse(lst$cex>0,1,19),col="black",cex=3*abs(lst$cex))
       }))
...
print(procerr.plot(MSH+MSH.sam,weight="stock.wt",type="n",rel=T))

# process error in terms of additional mortality
print(procerr.plot(MSH+MSH.sam,weight="stock.wt",type="mort",rel=F))
...
print(plot(MSH.retro,futureYrs=F))

# model parameters retrospective
retroParams(MSH.retro)

# stock trajectory retrospective
retroSelectivity(MSH.retro,2009:range(MSH)["maxyear"])
