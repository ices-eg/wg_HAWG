...
MSH.tun[[5]] <- window(MSH.tun[[3]],start=range(MSH)["minyear"],end=range(MSH)["maxyear"])
MSH.tun[[5]]@index[] <- MSH@catch.n

plot(MSH.tun[[5]],type="internal")
plot(MSH.tun[[1]],type="internal")# 6 and older
plot(MSH.tun[[2]],type="internal")# 2 and older
plot(MSH.tun[[3]],type="internal")# 2 and older
plot(MSH.tun[[4]],type="internal")# 3-8

## Trim indices to have the required ages
MSH.tun                         <- FLIndices("MS HERAS"=trim(MSH.tun[[1]],age=6:9),"WoS HERAS"=trim(MSH.tun[[2]],2:9),"IBTS_Q1"=trim(MSH.tun[[3]],age=2:9),"IBTS_Q4"=trim(MSH.tun[[4]],age=3:8))
...
# Catchabilities
MSH.ctrl@catchabilities["MS HERAS",ac(6:9)] <- c(rep(6,4))
MSH.ctrl@catchabilities["WoS HERAS",ac(2:9)]<- c(102:104,rep(104,5))
MSH.ctrl@catchabilities["IBTS_Q1",ac(2:9)]  <- c(202)
MSH.ctrl@catchabilities["IBTS_Q4",ac(3:8)]  <- c(303)
...
MSH.ctrl@obs.vars["MS HERAS",6:9]           <- c(rep(101,2),rep(102,2))
MSH.ctrl@obs.vars["WoS HERAS",2:9]          <- c(rep(202,4),rep(203,2),rep(204,2))
MSH.ctrl@obs.vars["IBTS_Q1",2:9]            <- c(302,rep(303,2),rep(304,3),rep(305,2))
MSH.ctrl@obs.vars["IBTS_Q4",3:8]            <- c(rep(403,6))

#- Correlation structure in the indices
MSH.ctrl@cor.obs["MS HERAS",6:8]            <- rep(1,3)
MSH.ctrl@cor.obs["WoS HERAS",]              <- rep(101,8)
MSH.ctrl@cor.obs["IBTS_Q1",2:8]             <- rep(202,7)
MSH.ctrl@cor.obs["IBTS_Q4",3:7]             <- rep(302,5)

#- Set cor flags
MSH.ctrl@cor.obs.Flag[c(2:5)]               <- as.factor("AR")
