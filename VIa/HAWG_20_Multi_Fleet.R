...
path<-"C://Work/HAWG2019/wg_HAWG/VIa/"
...
MSH@stock.wt[1,ac(2019)]    <- yearMeans(MSH@stock.wt[1,ac(2010:2018)])
...
### There is a problem with writing out the ages 7-9 in the 6s(s)7bc component
### I ended up just writing a separate file out for them
MSH.sam.south <- MSHm.sam[,,,2,]
sam.out.south <- FLSAM.out(MSHm[,,,,2,],MSH.tun,MSHm.sam, format="TABLE South")
write(sam.out.south,file.path(output.dir,"MSH_sam.out.south",sep="."))
...
res.dat$data <- res.dat$std.res
res.dat$std.res[is.na(res.dat$std.res)] <- 0 # why is there an NA?
p <- xyplot(age  ~ year |fleet,res.dat,
            cex=4*abs(res.dat$std.res)/max(abs(res.dat$std.res), na.rm=T)+0.1,
            col=ifelse(res.dat$std.res<0,"red","black"),
            pch=16,ylab="age (ring)")
print(p)
