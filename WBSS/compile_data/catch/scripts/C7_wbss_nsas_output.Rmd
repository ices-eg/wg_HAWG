...
library(dplyr)
dataPath<-"C:/HAWG/2018/WBSS/data/catch/resultsCatch/"
```

#Read-in
```{r}

canum3<-readRDS(paste(dataPath,"C6_wbss_nsas_canum_2017.rds", sep=""))
canum3$weca_kg<-canum3$weca_g/1000

canum4<-read.table(paste(dataPath,"canum_weca_2017_27.4.a.e.csv", sep=""), sep=",", header=T)
...
write.table(wbssFinal, paste(dataPath,"C7_wbss_input_fleet_2017.csv", sep=""), sep=",", row.names=F)
...
write.table(subset(nsasFinal,fleet %in% c("C","D")), paste(dataPath,"C7_nsas_input_fleet_2017.csv", sep=""), sep=",", row.names=F)
...
write.table(wbssFinalq, paste(dataPath,"C7_wbss_input_quarter_2017.csv", sep=""), sep=",", row.names=F)
...
write.table(wbssFinal, paste(dataPath,"wbbs_catch_input_model_2016.csv", sep=""), sep=",", row.names=F)
...
write.table(wbssWEST, paste(dataPath,"wbbs_catch_west_input_model_2016.csv", sep=""), sep=",", row.names=F)
```

#Cacth WBSS and NSAS
```{r}
wbss3<-readRDS(paste(dataPath,"3a_splitted_2016.rsd", sep=""))
...
write.table(prop, paste(dataPath,"wbbs_nsas_prop_catch_input_model_2016.csv", sep=""), sep=",", row.names=F)
```
