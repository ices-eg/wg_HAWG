...
froFile <- "FO_2018_herHAWG_Exchange.xls"
```
...
dnk<-read_in_canum(paste(rawDataPath, dnkFile, sep=""), sheets=c(6,7,8,9,10,11,12), noAgeClasses=9)

ger<-read_in_canum(paste(rawDataPath, gerFile, sep=""), sheets=c(7:13), noAgeClasses=9)

pol<-read_in_canum(paste(rawDataPath, polFile, sep=""), sheets=c(6:12), noAgeClasses=9)

swe<-read_in_canum(paste(rawDataPath,sweFile, sep=""), sheets=c(6:12), noAgeClasses=9)

nor<-read_in_canum(paste(rawDataPath, norFile, sep=""), sheets=c(6:12), noAgeClasses=9)

fro<-read_in_canum(paste(rawDataPath, froFile, sep=""), sheets=c(7), noAgeClasses=16)
...
dnkS<-read_in_samples(paste(rawDataPath, dnkFile, sep=""), sheets=c(6,7,8,9,10,11,12), firstLineSamples=27)

gerS<-read_in_samples(paste(rawDataPath, gerFile, sep=""), sheets=c(7:13), firstLineSamples=27)

polS<-read_in_samples(paste(rawDataPath, polFile, sep=""), sheets=c(6:12), firstLineSamples=27)

sweS<-read_in_samples(paste(rawDataPath, sweFile, sep=""), sheets=c(6:12), firstLineSamples=27)

norS<-read_in_samples(paste(rawDataPath, norFile, sep=""), sheets=c(6:12), firstLineSamples=27)

froS<-read_in_samples(paste(rawDataPath, froFile, sep=""), sheets=c(7), firstLineSamples=39)
```
...
#froC<-read_in_catch(paste(rawDataPath, froFile, sep=""), sheets=c(3), noAreas=29) - Template from WGWIDE used - don't work
```
...
canum$area<-ifelse(canum$area %in% c("SD 20","IIIaN", "IIIa"), "27.3.a.20",
...
canum$fleet<-ifelse(canum$area %in% c("27.3.a.20","27.3.a.21") & canum$fleet %in% c("C","Fleet-C","> 32 mm", "Fleet-A"), "C",
                    ifelse(canum$area %in% c("27.3.a.20","27.3.a.21") & canum$fleet %in% c("D","Fleet-D","< 32mm","< 32 mm"), "D",
                    ifelse(canum$area %in% c("27.3.c.22","27.3.b.23","27.3.d.24","all"),"F", canum$fleet)))
...
#Output data
```{r}
saveRDS(canum1, paste(dataPath, "C1_her2024_canum_without_imputations_2017.rds", sep=""))
write.table(canum3, paste(dataPath, "C1_her2024_canum_without_imputations_2017.csv", sep=""), sep=",", row.names=F, na="")

saveRDS(samp, paste(dataPath, "C1_her2024_samples_2017.rds", sep=""))
write.table(samp, paste(dataPath, "C1_her2024_samples_2017.csv", sep=""), sep=",", row.names=F, na="")

saveRDS(catch, paste(dataPath, "C1_her2024_catch_2017.rds", sep=""))
write.table(catch, paste(dataPath, "C1_her2024_catch_2017.csv", sep=""), sep=",", row.names=F, na="")

saveRDS(length1, paste(dataPath, "C1_her2024_length_2017.rds", sep=""))
write.table(length1, paste(dataPath, "C1_her2024_length_2017.csv", sep=""), sep=",", row.names=F, na="")
```
