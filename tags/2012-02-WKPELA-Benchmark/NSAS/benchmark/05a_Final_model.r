...
respref <- "05a_Final_model" #Prefix for output files
...
if(!file.exists(resfile) | !interactive()) {
  ...
  NSH.sam@name <- "NSAS Herring Final Model"
...
last.yr <- which.max(CV.yrs)
matplot(CV.yrs[-last.yr],CV.dat[-last.yr,],type="l",ylim=range(pretty(c(0,CV.dat))),yaxs="i",
    lty=1,xlab="Year",ylab="CV of estimate",main="Uncertainties of key parameters")
legend("topleft",legend=colnames(CV.dat),lty=1,col=1:6,bty="n")
...
print(xyplot(value.f ~ year,sel.pat,groups=sprintf("Age %02i",age),
         type="l",as.table=TRUE,auto.key=list(space="right",points=FALSE,lines=TRUE),
...
print(levelplot(sel ~ year*age,sel.pat,at=seq(0,2.75,length.out=100),
        ...
        xlab="Year",ylab="Age",main="Selectivity of the fishery by time")
...
#SRR plot
ssb.dat <- transform(ssb(NSH.sam),cohort=year,year=NULL)
rec.dat <- transform(rec(NSH.sam),cohort=year-1,year=NULL)
srr.dat <- merge(ssb.dat,rec.dat,by="cohort",suffixes=c(".ssb",".rec"))
ssb.lims <- 10^range(pretty(log10(c(ssb.dat$ubnd,ssb.dat$lbnd))))
rec.lims <- 10^range(pretty(log10(c(rec.dat$ubnd,rec.dat$lbnd))))
plot(srr.dat$value.ssb,srr.dat$value.rec,log="xy",type="n",
  xlim=ssb.lims,ylim=rec.lims,main="Stock recruitment curve",
  xlab="SSB (tonnes)",ylab="Recruitment (billions)")
arrows(srr.dat$value.ssb,srr.dat$lbnd.rec,srr.dat$value.ssb,
  srr.dat$ubnd.rec,length=0,col="lightgrey")
arrows(srr.dat$lbnd.ssb,srr.dat$value.rec,srr.dat$ubnd.ssb,
  srr.dat$value.rec,length=0,col="lightgrey")
points(srr.dat$value.ssb,srr.dat$value.rec,pch=16)

plot(srr.dat$value.ssb,srr.dat$value.rec,log="xy",type="n",
  xlim=ssb.lims,ylim=rec.lims,main="Stock recruitment curve",
  xlab="SSB (tonnes)",ylab="Recruitment (billions)")
text(srr.dat$value.ssb,srr.dat$value.rec,col=rainbow(nrow(srr.dat)),
  sprintf("%02i",srr.dat$cohort%%100),cex=0.9)

#Otolith plot
source(file.path("benchmark","oto.plot.r"))
plot.otolith(NSH.sam,year=2010,n=10000)
title(main="2010 Otolith plot",outer=TRUE)
plot.new()
